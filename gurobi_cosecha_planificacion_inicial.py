...
porcentaje_a_satisfacer = 0.45
dps_vino_a = vinos["A"].volumen * 1333 / perdidas_merma * porcentaje_a_satisfacer
dps_vino_b = vinos["B"].volumen * 1333 / perdidas_merma * porcentaje_a_satisfacer
dps_vino_c = vinos["C"].volumen * 1333 / perdidas_merma * porcentaje_a_satisfacer
dps_vino_d = vinos["D"].volumen * 1333 / perdidas_merma * porcentaje_a_satisfacer
dps_vino_e = vinos["E"].volumen * 1333 / perdidas_merma * porcentaje_a_satisfacer
...
m.setObjective((2.8 * t["A"] + 3.1 * t["B"] + 3.05 * t["C"] + 2.7 * t["D"] + 2.4 * t["E"])
    ...
    - sum(lotes[l].calcular_costo(d) * x[l, d] for l in L for d in D), GRB.MAXIMIZE)


# Restricciones
# Definición variable w
m.addConstrs(sum(x[l, d] * lotes[l].tn * 1000 * relacion[j, l] for l in L) == w[j, d]
        for j in J
        for d in D)

# Definición variable y
m.addConstrs(sum(w[j, d] for d in D) == sum(y[j, r, v] for r in R for v in V) for j in J)
...
# Mínimo de satisfacción de demanda por vino
