...
for semana in range(1,15):
    ...
    #Función Objetivo
    m.setObjective((2.8*t['A']+3.1*t['B']+3.05*t['C']+2.7*t['D']+2.4*t['E']) - sum(lotes[l].precio * lotes[l].tn *1000 * x[l,d] for l in L for d in D)- sum(lotes[l].calcular_costo(d) * x[l,d] for l in L for d in D), GRB.MAXIMIZE)
    ...
    m.addConstr(t['A'] * perdidas_merma <= vinos['A'].volumen* 1000 * porcentaje_demanda_semana)
    ...
    m.addConstr(t['B'] * perdidas_merma <= vinos['B'].volumen *1000 * porcentaje_demanda_semana)
    ...
    m.addConstr(t['C'] * perdidas_merma <= vinos['C'].volumen *1000 * porcentaje_demanda_semana)
    ...
    m.addConstr(t['D'] * perdidas_merma <= vinos['D'].volumen*1000 * porcentaje_demanda_semana)
    ...
    m.addConstr(t['E'] * perdidas_merma <= vinos['E'].volumen* 1000 * porcentaje_demanda_semana)
    ...
    #para mostrar el valor de todas las variables
    '''
    for v in m.getVars():
        if v.x != 0:
            print(v.varName, v.X)
