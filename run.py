...
if __name__ == '__main__':
    ...
    cantidad_lluvia = []
    for i in lotes_list: 
        cantidad_lluvia.append([dict_results[i][0]/simulaciones, dict_results[i][0]/simulaciones])
    lotes_df = pd.DataFrame(cantidad_lluvia, columns = ['7 Dias Antes Prom','7 Dias Despues Prom'])
    lotes_df.insert(0, "Lote", lotes_list, True)
    lotes_df.to_excel('docs/lluvia_lotes.xlsx', sheet_name="lluvias")   
    final = time.time()
